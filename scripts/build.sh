...
show_menu() {
    ...
    echo "+---------------------------------------+"
    echo ""
    echo "  Local development: build, install, and"
    echo "  run Untouchable on THIS Mac (unsigned)."
    echo "  To publish a signed, notarised release"
    echo "  for users, run scripts/release.sh."
    echo ""
    echo "  Flags (e.g. --build) are optional CI"
    echo "  shortcuts -- with no flags you get this"
    echo "  menu, which is all you need."
    echo ""
    echo "+---------------------------------------+"
